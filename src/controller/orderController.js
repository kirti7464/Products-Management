...
const createOrder = async function(req,res){    //userId===cart.userId--authorization
    try{
        ...
        if(!isValidRequestBody(req.body)){
            return res.status(400).send({status : false, message : "Please provide data for creating order"})
        }
        let data={
            status:"pending",
            cancellable:true
...
const updateOrder = async function(req,res){
    try{
        ...
        if(!isValidRequestBody(req.body)){
            return res.status(400).send({status : false, message : "Please provide data for updating order"})
        }
        if(!orderId){
